...
const ChatComponent = () => {
    ...
    const sendMessage = useCallback(async () => {
        ...
        try {
            ...
            const data = response.data;
            if (response && data) {
                const responseMessage = data.response || data.message || (typeof data === "string" ? data : JSON.stringify(data));
                setMessages((prev) => [
                    ...prev,
                    { message: responseMessage, sender: "agent" },
                ]);
                // Speak the agent's message immediately after it's received and processed
                if (voiceEnabled && responseMessage) {
                    // Ensure voice is enabled and the message is not empty
            ...
            const errorMessage = error.response?.data?.error || "Network or server error occurred.";
